	...
	<view class="uni-goods-nav">
		...
		<view class="uni-tab__cart-box flex">
			...
			<view v-for="(item,index) in buttonGroup" :key="index" :style="{color:item.color}" :class="{one:index==0,two:index==1}"
			 class="uni-tab__cart-button-right" @click="buttonClick(index,item)"><text class="uni-tab__cart-button-right-text">{{ item.text }}</text></view>
			<!-- </view> -->
		</view>
	</view>
</template>
	...
	export default {
		...
		props: {
			options: {
				...
				default () {
					return [{
							icon: '/static/img/shangpin_dianp.png',
							text: '店铺'
						},
						{
							icon: '/static/img/shangpin_lianxi.png',
							text: '联系客服'
						},
						{
